...
**✅ DONE & VERIFIED: Supabase backend / cross-device sync (§8a)** — merged (PR #6) + **end-to-end verified 2026-06-14**: signed in on two devices, real holdings (722.9 total units across T212 + Binance) pulled from `public.user_data` on the second device. KV-mirror of `Store` → `public.user_data`. Manual setup done: Google provider enabled in Supabase, callback redirect URI added to OAuth client `185610168060-…`.

**🐞 FIXED 2026-06-19: live Emma data stopped reading through after connecting (branch `claude/google-account-emma-sync-mi6uvk`).** Root cause: the June 13 "single Google login" change sourced the Sheets read token from Supabase's `session.provider_token`. Supabase only exposes that token *once* (on the OAuth callback) — never on a reload or its own session refresh — so the code cached it for 55 min and then silently fell back to mock data with no reconnect prompt. Fix: the Sheets read token now comes from a dedicated **Google Identity Services token client** (`GAuth`, uses `CLIENT_ID` only — no secret), which can **renew the token silently** (`prompt:''`) on boot and after a 401, so live data survives reloads and hourly expiry. Supabase is now **KV-sync/identity only**. Trade-off (accepted): authorising Sheets is its own Google popup, so it's no longer a strictly single tap.
